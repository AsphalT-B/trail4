...
export function VideoForm({ initialVideo }: VideoFormProps) {
    ...
    const [thumbnailPreview, setThumbnailPreview] = useState(initialVideo?.thumbnailUrl ?? "");
    const [videoFile, setVideoFile] = useState<File | null>(null);
    const [isUploading, setIsUploading] = useState(false);

    const isEditing = !!initialVideo;
    const isSaving = createVideo.isPending || updateVideo.isPending || isUploading;
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        const submit = async () => {
            const thumbnailUrl = thumbnailFile ? await uploadFile(thumbnailFile) : initialVideo?.thumbnailUrl ?? "";
            const videoUrl = videoFile ? await uploadFile(videoFile) : initialVideo?.videoUrl ?? "";
    ...
    return (
        <form onSubmit={handleSubmit} className="flex max-w-xl flex-col gap-5 rounded-2xl border border-line bg-white p-6 shadow-sm sm:p-8">
            <div>
            ...
            <div>
                ...
                {thumbnailFile && <p className="mt-1.5 text-xs text-slate">{thumbnailFile.name}</p>}
            </div>

            <div>
                <label className="mb-1.5 block text-sm font-medium text-ink">Video file</label>
                <label className="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-line px-4 py-2 text-sm font-medium text-ink transition hover:border-primary hover:text-primary">
                ...
                {videoFile && <p className="mt-1.5 text-xs text-slate">{videoFile.name}</p>}
                {!videoFile && isEditing && (
                    <p className="mt-1.5 text-xs text-slate">Leave empty to keep the existing video file.</p>
                )}
            </div>
